 ...
 * it the {@link com.termux.shared.termux.x11.ExtraKeysInfo } to load and display the extra keys. Read its class javadocs for more
 ...
 * and calls {@link ExtraKeysView#reload(com.termux.shared.termux.x11.ExtraKeysInfo).
 * The {@link com.termux.shared.termux.x11.ExtraKeysInfo } is created by TermuxAppSharedProperties.setExtraKeys().
...
public final class ExtraKeysView extends GridLayout {
    ...
    public interface IExtraKeysView {
         ...
         * for {@link #mRepetitiveKeys} and {@link com.termux.shared.termux.x11.ExtraKeyButton} that have a popup set.
         * However, this is not called for {@link #mSpecialButtons}, whose state can instead be read
         * via a call to {@link #readSpecialButton(com.termux.shared.termux.x11.SpecialButton, boolean)}.
         *
         * @param view The view that was clicked.
         * @param buttonInfo The {@link com.termux.shared.termux.x11.ExtraKeyButton} for the button that was clicked.
         *                   The button may be a {@link com.termux.shared.termux.x11.ExtraKeyButton#KEY_MACRO} set which can be
         *                   checked with a call to {@link com.termux.shared.termux.x11.ExtraKeyButton#macro}.
         * @param button The {@link Button} that was clicked.
         */
        void onExtraKeyButtonClick(View view, com.termux.shared.termux.x11.ExtraKeyButton buttonInfo, Button button);
         ...
         * so that {@link ExtraKeysView#performExtraKeyButtonHapticFeedback(View, com.termux.shared.termux.x11.ExtraKeyButton, Button)}
         * can handle it depending on system settings.
         */
        boolean performExtraKeyButtonHapticFeedback(View view, com.termux.shared.termux.x11.ExtraKeyButton buttonInfo, Button button);
    }
    ...
    /** The map for the {@link com.termux.shared.termux.x11.SpecialButton} and their {@link com.termux.shared.termux.x11.SpecialButtonState}. Defaults to
     * the one returned by {@link #getDefaultSpecialButtons(ExtraKeysView)}. */
    private Map<com.termux.shared.termux.x11.SpecialButton, com.termux.shared.termux.x11.SpecialButtonState> mSpecialButtons;

    /** The keys for the {@link com.termux.shared.termux.x11.SpecialButton} added to {@link #mSpecialButtons}. This is automatically
     ...
     * is long pressed. This is done by calling {@link IExtraKeysView#onExtraKeyButtonClick(View, com.termux.shared.termux.x11.ExtraKeyButton, Button)}
     * every {@link #mLongPressRepeatDelay} seconds after {@link #mLongPressTimeout} has passed.
     * The default keys are defined by {@link com.termux.shared.termux.x11.ExtraKeysConstants#PRIMARY_REPETITIVE_KEYS}.
     */
    ...
    /** The popup window shown if {@link com.termux.shared.termux.x11.ExtraKeyButton#popup} returns a {@code non-null} value
    ...
    public void setSpecialButtons(@NonNull Map<com.termux.shared.termux.x11.SpecialButton, com.termux.shared.termux.x11.SpecialButtonState> specialButtons) {
        mSpecialButtons = specialButtons;
        mSpecialButtonsKeys = this.mSpecialButtons.keySet().stream().map(com.termux.shared.termux.x11.SpecialButton::getKey).collect(Collectors.toSet());
    }
    ...
    @NonNull
    public Map<com.termux.shared.termux.x11.SpecialButton, com.termux.shared.termux.x11.SpecialButtonState> getDefaultSpecialButtons(ExtraKeysView extraKeysView) {
        return new HashMap<>() {{
            put(com.termux.shared.termux.x11.SpecialButton.CTRL, new com.termux.shared.termux.x11.SpecialButtonState(extraKeysView));
            put(com.termux.shared.termux.x11.SpecialButton.ALT, new com.termux.shared.termux.x11.SpecialButtonState(extraKeysView));
            put(com.termux.shared.termux.x11.SpecialButton.SHIFT, new com.termux.shared.termux.x11.SpecialButtonState(extraKeysView));
            put(com.termux.shared.termux.x11.SpecialButton.META, new com.termux.shared.termux.x11.SpecialButtonState(extraKeysView));
            put(com.termux.shared.termux.x11.SpecialButton.FN, new com.termux.shared.termux.x11.SpecialButtonState(extraKeysView));
        }};
    }
    ...
    public void reload() {
        ...
        for(com.termux.shared.termux.x11.SpecialButtonState state : mSpecialButtons.values())
            state.buttons = new ArrayList<>();

        removeAllViews();

        com.termux.shared.termux.x11.ExtraKeyButton[][] buttons = extraKeysInfo.getMatrix();
        ...
        for (int row = 0; row < buttons.length; row++) {
            for (int col = 0; col < buttons[row].length; col++) {
                final com.termux.shared.termux.x11.ExtraKeyButton buttonInfo = buttons[row][col];
    ...
    public void performExtraKeyButtonHapticFeedback(View view, com.termux.shared.termux.x11.ExtraKeyButton buttonInfo, Button button) {
        if (mExtraKeysViewClient != null) {
    ...
    public void onAnyExtraKeyButtonClick(View view, @NonNull com.termux.shared.termux.x11.ExtraKeyButton buttonInfo, Button button) {
        if (isSpecialButton(buttonInfo)) {
            if (mLongPressCount > 0) return;
            com.termux.shared.termux.x11.SpecialButtonState state = mSpecialButtons.get(com.termux.shared.termux.x11.SpecialButton.valueOf(buttonInfo.key));
    ...
    public void startScheduledExecutors(View view, com.termux.shared.termux.x11.ExtraKeyButton buttonInfo, Button button) {
        stopScheduledExecutors();
        mLongPressCount = 0;
    ...
    public class SpecialButtonsLongHoldRunnable implements Runnable {
        public final com.termux.shared.termux.x11.SpecialButtonState mState;

        public SpecialButtonsLongHoldRunnable(com.termux.shared.termux.x11.SpecialButtonState state) {
            mState = state;
        }
    ...
    void showPopup(View view, com.termux.shared.termux.x11.ExtraKeyButton extraButton) {
    ...
    /** Check whether a {@link com.termux.shared.termux.x11.ExtraKeyButton} is a {@link com.termux.shared.termux.x11.SpecialButton}. */
    public boolean isSpecialButton(ExtraKeyButton button) {
        return mSpecialButtonsKeys.contains(button.key);
    }

    /**
     * Read whether {@link com.termux.shared.termux.x11.SpecialButton} registered in {@link #mSpecialButtons} is active or not.
     *
     * @param specialButton The {@link com.termux.shared.termux.x11.SpecialButton} to read.
     * @param autoSetInActive Set to {@code true} if {@link com.termux.shared.termux.x11.SpecialButtonState#isActive} should be
     ...
     */
    @Nullable
    public Boolean readSpecialButton(com.termux.shared.termux.x11.SpecialButton specialButton, boolean autoSetInActive) {
        com.termux.shared.termux.x11.SpecialButtonState state = mSpecialButtons.get(specialButton);
